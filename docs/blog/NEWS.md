...
- Déplacement des assets de marque dans `assets/brand`.
- Création d'une couche UI commune dans `packages/ui`.
- Extraction des corpus de jeux dans `packages/corpus`.
- Ajout d'utilitaires partagés dans `packages/game-utils`.
- Découpage de Lille-Mêle en HTML, CSS, JavaScript et données séparées.
- Ajout de scripts de validation pour le JavaScript, le corpus et les pages statiques.
