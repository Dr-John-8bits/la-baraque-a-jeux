...
> **Statut : en ligne (v1, variante A) + corpus poli.** Jeu **codé, testé et branché au portail** (`app.js`/`index.html`/`styles.css`) : tirage déterministe de 5 faits aux années bien séparées, réordonnancement flèches + glisser-déposer, **3 essais avec feedback** (les cartes bien placées passent au vert, se verrouillent et révèlent leur date ; réordonnancement lock-aware), révélation finale (dates + fiche + source), partage spoiler-free (1 rangée 🟩/⬛ par essai), calepin partagé, première-aide, compte à rebours. Corpus = **34 faits datés sourcés et curés** (8 graines + 26 Wikidata CC0 via `tools/build-events-from-wikidata.mjs`, `npm run build:frise-events`). Curation resserrée pour la **reconnaissance** : plancher de notoriété (naissances/événements), exclusion des fondations d'institutions / stations de métro / cultuel, `DENY_QIDS` + `LABEL_OVERRIDE` éditoriaux. Contenu = repères lillois (Bouvines, Vieille Bourse, Citadelle, Mongy, VAL, Euralille, Louvre-Lens…) + naissances célèbres (de Gaulle, Perrin, Noiret, Varane…). **Restent** : (1) **brancher au portail** (grille 4 jeux, cf. §9) + SW precache + validateur de pages ; (2) étendre éventuellement le corpus (le tool est re-jouable) ; (3) polir les blurbs auto.
