...
- 2026-06-14 : Bloc 3 (stabilisation). Playtest mobile complet (0 erreur console ; LocalStorage corrompu géré). Corrections issues d'une chasse aux bugs : rejet d'un état sauvegardé au `status` aberrant, fiabilisation du décompte `stats.played` quand le stockage est bloqué, `max-height` des suggestions en `clamp(50svh)`, fermeture des suggestions à l'ouverture d'une modale. Suggestions ouvertes vers le bas, Échap ferme sans vider le champ. Tuile portail passée à « Métro / Jouer ». Relecture éditoriale des 60 fiches reportée à la phase éditoriale. `npm run check` et `npm run test:browser` OK.
